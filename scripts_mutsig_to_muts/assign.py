...
def get_real_mut(df):
    ...
    mut = '{}{}{}'.format(first, second, third)
    reversed = '{}{}{}'.format(reverse[third], reverse[second], reverse[first])
...
def load_cosmic(cosmic_file):
    ...
    renames = {c: '.'.join(c.split()) for c in W.columns if c.startswith('SBS')}
...
def assign_probability(inputfile, weights, cosmic_file, output_dir):
    ...
    selected = [col for col in results_new.columns if col.startswith('SBS')]
    ...
    # open all files
    all_files_out = {s: open(os.path.join(output_dir, '{}.tsv'.format(s.replace('.', '_'))), 'wt')
...
@click.argument('cosmic_file', metavar='<SIGNATURES FILE>', type=click.Path(exists=True))
...
def cli(input, weights, cosmic_file, output, verbose):
    """
    ...
    assign_probability(input, weights, cosmic_file, output)
